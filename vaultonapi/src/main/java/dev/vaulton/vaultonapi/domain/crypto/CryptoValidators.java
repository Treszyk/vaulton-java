...
/**
 * Provides domain-level validation policies for encrypted data.
 * These methods ensure that cryptographic payloads meet context-specific
 * requirements (e.g., exact lengths for master keys).
 */
public final class CryptoValidators {
    ...
    public static boolean isValidVaultEntry(EncryptedValue v) {
        ...
        return v.cipherText().length >= 1 && v.cipherText().length <= CryptoConstants.MAX_ENTRY_CIPHERTEXT_BYTES;
    }
}
