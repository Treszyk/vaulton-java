...
class EncryptedValueTest {
    ...
    void shouldThrowWhenInvalidCryptoSize() {
        ...
        assertThrows(IllegalArgumentException.class, () -> {
            new EncryptedValue(
                    new SecureBuffer("ąąąąąą".getBytes(StandardCharsets.UTF_8)),
                    new SecureBuffer(new String(new char[CryptoConstants.MAX_ENTRY_CIPHERTEXT_BYTES+1]).replace('\0', 'ą').getBytes(StandardCharsets.UTF_8)), // 4098 bytes instead of 4096
                    new SecureBuffer("ąąąąąąąą".getBytes(StandardCharsets.UTF_8))
            );
        });
    }
    ...
    void shouldCreateCorrectlyWhenValidCryptoSize() {
        assertDoesNotThrow(() -> {
            new EncryptedValue(
                    ...
                    new SecureBuffer(new String(new char[CryptoConstants.MK_LEN]).replace('\0', 'ą').getBytes(StandardCharsets.UTF_8)), // 4096 bytes
